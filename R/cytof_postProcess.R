            ...
            if(!is.null(clusterData) && length(clusterData) > 0){
                for(j in 1:length(clusterData)){
                    if(!is.null(clusterData[[j]])){
                        ...
                        xlab <- colnames(dataij)[1]
                        ylab <- colnames(dataij)[2]
                        cluster <- colnames(dataij)[3]
                        sample <- colnames(dataij)[4]
...
#' 
#' @param data The data frame of cluster results
#' @param xlab The name for the x axis in input \code{data}
#' @param ylab The name for the y axis in input \code{data}
#' @param clueter the name of cluter column in input \code{data}
#' @param sample the name of the sample column in input \code{data}
#' @param title the title name of the plot.
#' @param type plot type, 1 indicates combined plot, 2 indicated grid facet plot by samples
#' @param point_size the size of the dot.
#' @return the ggplot object of the scatter dot plot
#' @export
#' @examples
...
cytof_clusterPlot <- function(data, xlab, ylab, cluster, sample, 
    ...
    if(all(c(missing(xlab),missing(ylab),missing(cluster),missing(sample)))){
        xlab <- colnames(data)[1]
        ylab <- colnames(data)[2]
        cluster <- colnames(data)[3]
        sample <- colnames(data)[4]
    }
    ...
    if(!all(c(xlab, ylab, cluster) %in% colnames(data))){
        stop("undefined parameters in the data")
    }
    ...
    if(type == 1){
        cp <- ggplot(data, aes_string(x = xlab, y = ylab, colour = cluster, shape = sample)) + 
            geom_point(size = point_size) + #geom_rug(position = "jitter", size = 0.2) + 
            theme_bw() + scale_shape_manual(values = shape_value) + 
            scale_colour_manual(values = rainbow(cluster_num)) + 
            theme(legend.position = "bottom") + 
            xlab(xlab) + ylab(ylab) + ggtitle(paste(title, "scatter plot", sep = " ")) + 
            coord_fixed() + guides(colour = guide_legend(nrow = col_legend_row, override.aes = list(size = 4)), 
        ...
        cp <- ggplot(data, aes_string(x = xlab, y = ylab, colour = cluster)) + 
            ...
            geom_point(size = point_size - 0.05 * sample_num) + theme_bw() + #geom_rug(position = "jitter", size = 0.2) + 
            scale_colour_manual(values = rainbow(cluster_num)) + 
            theme(legend.position = "bottom") + xlab(xlab) + ylab(ylab) + 
            ggtitle(paste(title, "grid plot", sep = " ")) + coord_fixed() + 
...
#' 
#' @param data data matrix for heat map
...
cytof_heatmap <- function(data, baseName = "Cluster", scaleMethod = "none") {
    ...
    cex_row_label <- (9 - ceiling(nrow(data)/10))/10
    cex_col_label <- (9 - ceiling(ncol(data)/10))/10
