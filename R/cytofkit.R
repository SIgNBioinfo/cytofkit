...
#' 
#'  - Using function \code{\code{cytof_heatmap}} to generate heat map to visualize the mean expression of every marker in every cluster. 
#'  This heat maps is useful to interrogate marker expression to identify each cluster's defining markers. 
#' 
#'  - Using function \code{\code{cytof_progressionPlot}} to visualize the expression patter of selected markers against the estimated 
#'  cellular progression order.
#'  
#'  - Using function \code{\code{cytof_addToFCS}} to add any dimension reduced data, cluster results, progression data into the original FCS files,
...
cytofkit <- function(fcsFiles = getwd(), markers = NULL, mergeMethod = "ceil", fixedNum = 10000, 
                     ...
                     scaleTo = NULL, q = 0.05, 
                     dimReductionMethod = "tsne", clusterMethods = "densVM", 
    ...
    if (is.null(markers)) {
        ...
        if (file.exists(markers)) {
            markers <- as.character(read.table(markersFile, sep = "\t", 
                                               header = TRUE)[, 1])
        }
    } 
